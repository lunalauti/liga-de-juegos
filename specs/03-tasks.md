...
- [ ] **T5.5** Ping externo cada 10 min contra `/health` para el cold start. §7 **Desbloqueado** — ya existe `https://liga-de-juegos-api.onrender.com/health`. Falta darlo de alta en cron-job.org (gratis, no necesita tarjeta).
- [ ] **T5.6** **Prueba de aceptación del MVP** con los criterios de `01-requirements.md` §7, hecha desde el celular. **Desbloqueado** — sistema desplegado de punta a punta y verificado (login con sesión real, home de "Hoy" mostrando datos reales del grupo "La banda del crucigrama"). Falta la pasada formal desde el celular contra los criterios de aceptación.
