...
| D10 | ¿Un resultado importado con un tiempo real peor que la penalización se capea igual que en la carga manual? | **Sin resolver — encontrado con un dato real del usuario.** RF-6b es explícito para la carga manual: "un tiempo peor que la penalización se carga como DNF, no aporta nada peor". La importación (RF-6) no aplica esa regla: guarda `ln.seconds` tal cual venga. Con un Sudoku Avanzado real de 58:37 contra una penalización de 45:00, hoy el sistema carga el tiempo real completo — a ese jugador le conviene más, en términos de puntaje, haberse rendido que haber terminado tarde. Ningún RF dice explícitamente qué hacer acá; asumido pendiente hasta que el usuario decida entre: (a) aplicar el mismo cap también a lo importado, aunque "esconda" el tiempo real verificado, o (b) dejar el tiempo real tal cual llega, aceptando que terminar tarde puede constarle más que un DNF. | Medio — cambia el ranking de quien ya cargó un resultado así |
