...
2. **Cargar tiempos** — Tiene dos caminos y el diseño tiene que dejar clarísimo cuál es el principal:
   - **Pegar el link** (principal): un campo grande arriba de todo, "Pegá el link de tu resultado". Al pegarlo aparece una tarjeta de preview con lo detectado —juego, fecha, tiempo, y un sello de *verificado*— y un botón para confirmar. Diseñá también el estado de error: "ese link ya lo cargó Martín".
   - **Cargar a mano** (secundario, visualmente más abajo o plegado): los tres juegos en una pantalla, cada uno con input de tiempo grande y numérico y un toggle "No lo terminé" que reemplaza el input por la penalización correspondiente (20 / 40 / 45 min) indicada como castigo. Selector de fecha discreto arriba (default: hoy) y un solo botón de guardar.
   Un tiempo importado por link es **verificado** y uno tipeado a mano no: hace falta un distintivo visual chico y consistente para esa diferencia, que se repite en el ranking y en el detalle del día. Que sea una marca de confianza, no una acusación.
3. **Ranking** — Tabs Semana / Mes. Podio de los tres primeros con tratamiento visual propio, y debajo la tabla completa: posición, avatar, nombre, tiempo total, y desglose por juego. Mi fila siempre destacada. Chips secundarios por fila: cantidad de DNF, victorias del día, racha y si sus tiempos están verificados. Debe leerse de un vistazo en un celular sin scroll horizontal.
